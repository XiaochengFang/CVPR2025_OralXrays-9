...
class DynamicAssigner(BaseAssigner):
    ...
    def __init__(self,
                 pos_iou_thr,
                 neg_iou_thr,
                 low_quality_iou_thr=0.5,
                 base_pos_iou_thr=0.55,
                 r=0.15,
                 base_size=12,
                 scale_ratio=1,
                 min_pos_iou=.0,
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None, scale_ratio=1.0):
        """Assign gt to bboxes.
        ...
        # assign_result, ign_inds = self.assign_wrt_overlaps(overlaps, gt_areas, gt_labels, num_base_anchors, scale_ratio=scale_ratio)
        # if assign_on_cpu:
        ...
        assign_result = self.assign_wrt_overlaps(overlaps, gt_areas, gt_labels, scale_ratio=scale_ratio)
        if assign_on_cpu:
            assign_result.gt_inds = assign_result.gt_inds.to(device)
            assign_result.max_overlaps = assign_result.max_overlaps.to(device)
            if assign_result.labels is not None:
    ...
    def assign_wrt_overlaps(self, overlaps, gt_areas, gt_labels=None, scale_ratio=1.0):
        ...
        #print("-------------------gt_pos_thrs",gt_pos_thrs)
        #print("-------------------max_overlaps",max_overlaps)
        #print("-------------------potential_low_thr",potential_low_thr)
    ...
    def get_gt_pos_thrs(self, areas, scale_ratio):
        ...
        thrs = torch.min(torch.tensor(self.pos_iou_thr).cuda(), thrs)
        return thrs