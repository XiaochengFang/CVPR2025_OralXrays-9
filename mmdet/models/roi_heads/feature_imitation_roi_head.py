...
class BalSCL(nn.Module):
    def __init__(self, cls_num, temperature):
...
class FIRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        self.temperature = temperature
        # 初始化loss
        self.criterion_scl = BalSCL(self.num_classes+1, self.temperature).cuda()

        self.feature_fc = nn.Sequential(nn.Linear(1024, 1024 // 2), nn.BatchNorm1d(1024 // 2), nn.ReLU(inplace=True),
                                        nn.Linear(1024 // 2, 1024))
        self.centure_fc = nn.Sequential(nn.Linear(1024, 1024//2), nn.BatchNorm1d(1024//2), nn.ReLU(inplace=True),
                                     nn.Linear(1024//2, 1024))
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            pos_coords = (
            ori_coords, ori_labels, ori_bbox_targets, ori_bbox_feats, 'rcnn')  # 包含了建议框、标签、边界框回归目标和边界框特征的元组
    ...
    def _bbox_forward(self, x, rois):
        ...
        cls_score, bbox_pred, cls_embed = self.bbox_head(bbox_feats)
    ...
    def _bbox_forward_train(self, x, sampling_results,
        ...
        rois = bbox2roi([res.bboxes for res in sampling_results])# 将采样结果转换为 ROI
        bbox_results = self._bbox_forward(x, rois) # 使用 _bbox_forward 函数计算 ROI 特征和相关结果
        # print("bbox_results['cls_score']", bbox_results['cls_score'].shape)
        bbox_targets = self.bbox_head.get_targets(sampling_results, gt_bboxes,
                                                  gt_labels, self.train_cfg)# 获取边框目标

        # print("bbox_targets", bbox_targets)
        ori_label = bbox_targets[0]
        ...
        # 计算长尾分布对比loss
        cls_embed = bbox_results['cls_embed']
        # print("cls_embed shape", cls_embed.shape)
        # print("self.bbox_head.fc_cls.weight shape", self.bbox_head.fc_cls.weight.shape)

        cls_embed_mlp = self.feature_fc(cls_embed)
        centers_mlp = self.centure_fc(self.bbox_head.fc_cls.weight)
        
        cls_embed_norm = F.normalize(cls_embed_mlp, dim=1)
        centers_norm = F.normalize(centers_mlp, dim=1)
        cls_embed_unsq = cls_embed_norm.unsqueeze(1)
        bcl_loss = self.criterion_scl(centers_norm, cls_embed_unsq, ori_label)
        loss_bbox.update(loss_bcl=bcl_loss)
        
        # 更新结果字典并返回
        bbox_results.update(loss_bbox=loss_bbox)
        bbox_results.update(bbox_targets=bbox_targets)
        # print("bbox_results loss", bbox_results)
        return bbox_results
